...
/* this works
static uint64_t insecure_seed = 0xdeadbeefcafebabe;
...
/* from random.cpp */
FastRandomContext::FastRandomContext(const uint256& seed) : requires_seed(false)
...
int main()
{
    ...
    for (int i = 0; i <= 32; ++i) {
        ...
        SHA256D64(out2, in, i);
        if(memcmp(out1, out2, 32 * i) == 0){
		//printf("memcmp for i=%d okay\n", i);
		pass++;
	} else {
		printf("MEMCMP for i=%d FAILED\n",i);
		for (int j = 0; j < i; ++j){
			printf(" %2x", out1[j]);
		}
		printf("\n");
    ...
    if (fail > 0) {
	    printf("test FAIL:%d (pass count %d)\n", fail, pass);
	    exit(1);
    } else {
	    printf("test PASS:%d\n", pass);
    }
